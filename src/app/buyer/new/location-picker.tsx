...
async function reverseGeocode(lat: number, lng: number) {
  const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}`, { headers: { Accept: "application/json" } });
  if (!response.ok) throw new Error("Could not find this address.");
  const result = (await response.json()) as { display_name?: string };
  return result.display_name ?? `Pinned location (${lat.toFixed(5)}, ${lng.toFixed(5)})`;
}
...
async function findAddresses(query: string) {
  ...
  return results.map((result) => ({ label: result.display_name, lat: Number(result.lat), lng: Number(result.lon) }));
}
...
export function LocationPicker() {
  ...
  function updatePosition(nextPosition: { lat: number; lng: number }) {
    ...
    setSaved(false);
    setLocationError("");
    void reverseGeocode(nextPosition.lat, nextPosition.lng).then(setAddress).catch(() => undefined);
  }
  ...
  async function searchAddress() {
    ...
    try {
      ...
      setLocationError(error instanceof Error ? error.message : "Could not search for this address.");
    } finally {
      setBusy("");
    }
  }
  ...
  function useCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition(async ({ coords }) => {
      ...
      try {
        ...
        setAddress(`Pinned location (${coords.latitude.toFixed(5)}, ${coords.longitude.toFixed(5)})`);
      } finally {
        setBusy("");
      }
    }, () => {
      setLocationError("We could not access your location. You can search for an address or drag the pin instead.");
      setBusy("");
  ...
  function saveLocation() {
    ...
    const nextSavedLocations = [nextLocation, ...savedLocations.filter((location) => location.label !== nextLocation.label)].slice(0, 3);
    window.localStorage.setItem(SAVED_LOCATIONS_KEY, JSON.stringify(nextSavedLocations));
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-4">
        <label className="text-sm font-medium text-slate-800" htmlFor="delivery-location">Where should we deliver?</label>
        <button className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 hover:text-emerald-800" type="button" onClick={useCurrentLocation} disabled={busy === "locate"}>
          <LocateFixed className="size-3.5" />{busy === "locate" ? "Locating..." : "Use my location"}
        </button>
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <p className="min-w-0 text-xs text-slate-500"><span className="font-medium text-slate-700">Selected:</span> {address || "Search or move the pin to choose a location"}</p>
        <Button className="shrink-0" type="button" variant="ghost" onClick={saveLocation} disabled={!address.trim() || saved}>
